...
const categoryKeywords: Record<string, string[]> = {
  "DePIN": ["depin"], // Prioritize DePIN by placing it first
  "DeFi": ["defi", "finance", "uniswap", "financial", "exchange"],
  "Technology": ["technology", "tech", "technical", "tracking", "network"], // Removed blockchain to avoid conflicts
...
export const determineCategory = (description: string): string => {
  if (!description) return "Technology"; // Safeguard for empty descriptions
  
  const lowerDesc = description.toLowerCase();
  
  // First priority check: Look for "depin" variations explicitly
  if (lowerDesc.includes("depin")) {
    return "DePIN";
  }
  
  // Second priority check: Standard keyword matching
...
export const fetchPodcasts = async (): Promise<Podcast[]> => {
  try {
    ...
    const podcasts = await response.json() as Podcast[];
    
    // Ensure categories are properly assigned - recalculate them when fetching
...
export const fetchPodcastById = async (id: string): Promise<Podcast | null> => {
  try {
    ...
    const podcast = await response.json() as Podcast;
    
    // Ensure the category is properly assigned
    return {
      ...podcast,
      category: determineCategory(podcast.description)
    };
